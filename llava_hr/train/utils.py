...
@torch.no_grad()
def _first_forward_prop_seco(model, inputs_embeds_list, labels_list, attention_mask, valid_label_count, output_attentions=False):
    ...
    if output_attentions:
        bsz = inputs_embeds_list[0].shape[0]
        seq_len = sum([embeds.shape[1] for embeds in inputs_embeds_list])
        accum_attn = torch.zeros(size=(bsz,num_layers,seq_len,seq_len), dtype=inputs_embeds_list[0].dtype, device='cuda')
    else:
        accum_attn = None
    ...
    for i, (chunk_embeds, chunk_labels) in enumerate(zip(inputs_embeds_list, labels_list)):
        ...
        if output_attentions:
            attn = torch.stack(outputs.attentions).mean(dim=2).transpose_(0,1)
            start = chunk_size * i
            end = start + attn.shape[-2]
            accum_attn[..., start: end, :attn.shape[-1]] += attn

        accum_loss += outputs['loss'].sum() / valid_label_count

    return accum_loss, seco_cache, accum_attn.tril_(0)
...
@torch.no_grad()
def _sample_tokens_v2(inputs_embeds, labels, attention_mask, image_masks, chunk_budget, bwd_chunk_size, attentions=None):
    text_masks = attention_mask.clone()
    text_masks[image_masks] = False
    indices = []
    valid_length = []
    ...
    if attentions is None:
        attentions = [None] * len(text_masks)

    for text_mask, image_mask, attn in zip(text_masks, image_masks, attentions):
        full_ids = torch.arange(text_masks.shape[1], device='cuda')
        text_ids = full_ids[text_mask]
        image_ids = full_ids[image_mask]
        ...
        """剩下的budget是image的"""
        remain_budget = chunk_budget * bwd_chunk_size - select_ids.numel()
        if attn is not None or image_ids.numel() == 0:
            # layer * text_query, image_key
            prompt_ids = text_ids < image_ids.min()
            text_ids = text_ids[~prompt_ids]

            text_image_attn = attn[:, text_ids.long(), :][:, :, image_ids.long()].flatten(0,1)
            topk_indices = torch.topk(text_image_attn, k=remain_budget, dim=-1).indices
            topk_masks = torch.zeros_like(text_image_attn, dtype=torch.bool).scatter(dim=-1, index=topk_indices, value=True)
            vote = topk_masks.sum(dim=0)

            vote = vote.unflatten(0, (-1, IMAGE_CHUNK_SIZE))
            selector = vote.argsort(dim=-1, descending=True)[:, :remain_budget // vote.shape[0]]
            offset = torch.arange(vote.shape[0], device=selector.device, dtype=selector.dtype)[:, None] * IMAGE_CHUNK_SIZE
            selector = (selector + offset).flatten()
            ...
            # plt.figure()
            # plt.subplot(211)
            # plt.imshow(topk_masks.unflatten(0, (32, -1))[0].cpu().float().numpy())
            # plt.subplot(212)

            # choice_visualize = torch.zeros_like(topk_masks.unflatten(0, (32, -1))[0], dtype=torch.bool)
            # choice_indices = selector[None, :].expand(choice_visualize.shape[0], -1)
            # choice_visualize.scatter_(dim=-1, index=choice_indices, value=True)
            # plt.imshow(choice_visualize.cpu().numpy())
            # plt.savefig("attn.jpg", dpi=960)
        else:
            selector = torch.randperm(image_ids.numel(), device='cuda')[:remain_budget]
        select_ids = torch.cat([select_ids, image_ids[selector]], dim=0)
        valid_length.append(select_ids.numel())
