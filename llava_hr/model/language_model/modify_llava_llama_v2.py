...
@torch.no_grad()
def _build_ids_and_mask(inputs_embeds, attention_mask):
    # 得到batch size以及sequence length
    ...
    return posids, combined_attention_mask


def _compute_loss(logits, labels, vocab_size, shift_label=True, is_reduce=False):
    if shift_label:
...
def model_forward(
    ...
    posids, attention_mask = _build_ids_and_mask(
        inputs_embeds, 
        attention_mask)
...
def layer_forward(
    ...
    if torch.is_grad_enabled():
        ...
        r.register_hook(partial(_set_to_incomming_grad, incomming_grad=grad))
        backward(r.sum())
